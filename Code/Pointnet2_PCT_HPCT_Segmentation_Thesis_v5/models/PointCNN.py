...
from models.XConv import XConv, View
import math
import torch
import sys
sys.path.append("..")
...
class get_model(nn.Module):
    ...
    def __init__(self, setting:dict):
        ...
        for layer_idx, layer_param in enumerate(self.setting['xconv_params']):
            ...
            last_layer_flag = (layer_idx == (len(self.setting['xconv_params'])-1) )
        ...
        if "xdconv_params" in setting:
            for layer_idx, layer_param in enumerate(setting["xdconv_params"]):
                ...
                self.net.add_module(name=layer_name, module=nn.Sequential(nn.Linear(self.xconv_output_dim[qrs_layer_idx+1]+C, C).to(config.device),View((-1,C,P)),nn.BatchNorm1d(num_features=C).to(config.device),View((-1,P,C))).to(config.device))
                self.xconv_output_dim.append(C)
    ...
    def forward(self, input,return_intermediate=False):
        ...
        if 'xdconv_params' in self.setting:
            for layer_idx, layer_param in enumerate(self.setting["xdconv_params"]):
                ...
                layer_name = "xdconv_dense{}".format(layer_idx)
                fts_fuse = self.net._modules[layer_name].forward(fts_concat)
                layer_pts.append(qrs)
...
def modelnet_10_x3_l4()->get_model:
    setting={}
...
# 我们要用的！
def shapenet_x8_2048_fps()->get_model:
    setting={}
    setting["num_classes"] = 2
    x = 8
    xconv_param_name = ('K', 'D', 'P', 'C', 'links')
    setting["xconv_params"] = [dict(zip(xconv_param_name, xconv_param)) for xconv_param in
                               [(8, 1, 2048, 32 * x, []),
    ...
    setting["task"] = "seg"
    ###### Do not change this
    setting['fts_is_None'] = True
    ###### Do not change this
    setting["with_X_transformation"] = True
    return get_model(setting)

def s3dis_x8_2048_fps()->get_model:
    setting={}
...
def scannet_x8_2048_fps4()->get_model:
    setting={}
